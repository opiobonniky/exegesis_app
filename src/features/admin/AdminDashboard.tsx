...
import { getTriviaAdminOverview, TriviaAdminOverview } from '../trivia/services/triviaApi';
...
import {
  isRtlLanguage,
  getLocale,
} from '../../component/language-translation/localeUtils';
import LanguagePickerModal, {
  FLAGS,
  NATIVE_NAMES,
} from '../../component/LanguagePickerModal';
...
const KpiCard: React.FC<{
  ...
  <View style={[kpiStyles.card, { backgroundColor: bgColor }]}>
    ...
    >
      <View style={[kpiStyles.iconWrap, { backgroundColor: `${color}25` }]}>
        {icon}
      </View>
      {trend !== undefined && (
        <View
    ...
    <Text
      style={[kpiStyles.value, { color, textAlign: isRtl ? 'right' : 'left' }]}
    >
      {value}
    </Text>
    <Text
      style={[
        kpiStyles.label,
        { color, opacity: 0.7, textAlign: isRtl ? 'right' : 'left' },
      ]}
    >
      {label}
    </Text>
...
const Drawer: React.FC<{
  ...
  const items = [
    ...
    },
    {
      ...
      icon: BookText,
      routeKey: 'adminDailyExegesis',
    },
    {
      id: 'adminTrivia',
      label: 'Bible Trivia',
      icon: HelpCircle,
      routeKey: 'adminTrivia',
    },
    {
      id: 'adminTriviaPerformance',
      label: 'Trivia Performance',
      icon: BarChart3,
      routeKey: 'adminTriviaPerformance',
    },
    {
      id: 'adminStudyTools',
      label: 'Study Tools',
      icon: ScrollText,
      routeKey: 'adminStudyTools',
    },
    {
      id: 'adminBookPrologues',
      label: 'Book Prologues',
      icon: BookOpen,
      routeKey: 'adminBookPrologues',
    },
    {
      id: 'adminPlans',
    ...
    {
      id: 'adminJournalModeration',
      label: 'Journal Moderation',
      icon: ShieldCheck,
    ...
    <View style={drawerStyles.overlay} pointerEvents="box-none">
      ...
      >
        <View style={{ paddingTop: headerHeight }}>
          ...
          >
            ...
            <View style={{ flex: 1, marginHorizontal: 14 }}>
              ...
              >
                ...
                <Text
                  style={[drawerStyles.roleText, { color: theme.drawerMuted }]}
                >
                  {ac?.superAdminBadge || 'Super Admin'}
                </Text>
              </View>
            </View>
          ...
          <View
            style={[
              drawerStyles.headerGlow,
              { backgroundColor: theme.drawerActive + '12' },
            ]}
          />
        </View>
        ...
        >
          <Text
            style={[
              drawerStyles.navSection,
              { color: theme.drawerMuted, textAlign: isRtl ? 'right' : 'left' },
            ]}
          >
          ...
          {items.map(item => {
            ...
            return (
              ...
              >
                <View
                  style={[
                    drawerStyles.navIconWrap,
                    {
                      backgroundColor: isActive
                        ? `${theme.drawerActive}18`
                        : 'transparent',
                    },
                  ]}
                >
          ...
          >
            ...
            <Text
              style={[
                drawerStyles.navLabel,
                { color: '#e05555', textAlign: isRtl ? 'right' : 'left' },
              ]}
            >
              {ac?.signOut || 'Sign Out'}
            </Text>
        ...
        >
          ...
          >
            <View
              style={[
                drawerStyles.footerDot,
                { backgroundColor: theme.drawerActive + '40' },
              ]}
            />
            <Text
              style={[
                drawerStyles.footerText,
                {
...
const AdminDashboard: React.FC = () => {
  ...
  const [triviaOverview, setTriviaOverview] = useState<TriviaAdminOverview | null>(null);
  ...
  const [translationPickerVisible, setTranslationPickerVisible] =
    useState(false);
  ...
  }, [isRtl, overlayOpacity, translateX]);
  ...
  const fetchStats = useCallback(async () => {
    try {
      const [data, triviaData] = await Promise.all([
        getAdminDashboardStats(),
        getTriviaAdminOverview(),
      ]);
      setStats(data);
      setTriviaOverview(triviaData);
    } catch (error) {
  ...
  const loadTranslationOptions = useCallback(async () => {
    try {
      ...
      setTranslationOptions(
        list.map((t: any) => ({ id: t.frontendId, name: t.name })),
      );
    } catch (error) {
  ...
  const userGrowthTrend = stats?.totalUsers
    ? Math.round((stats.newUsersThisMonth / stats.totalUsers) * 100)
    : 0;
  const activeUserTrend = stats?.totalUsers
  ...
  const avgDailyNew = stats
    ? Math.round(stats.newUsersThisMonth / dayOfMonth)
    : 0;
    ...
    return (
      <View style={[s.root, { backgroundColor: theme.bg }]}>
        ...
        <View style={{ padding: 16 }}>
          <SkeletonBlock
            height={160}
            style={{ borderRadius: 22, marginBottom: 16 }}
          />
    ...
    <View style={[s.root, { backgroundColor: theme.bg }]}>
      ...
      >
        ...
        <View style={s.topBarCenter}>
          <Text style={[s.topBarTitle, { color: theme.text }]}>
      ...
      >
        ...
        <View style={[s.heroCard, { backgroundColor: theme.accent }]}>
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              <Text
                style={[s.heroSub, { textAlign: isRtl ? 'right' : 'left' }]}
              >
                {today}
              </Text>
            </View>
          ...
          >
            ...
            >
              ...
              <Text style={s.heroMetricLabel}>
                {ac?.newThisMonth || 'New this month'}
              </Text>
            </View>
            <View
              style={{
        ...
        <View style={[s.dailyCard, { backgroundColor: theme.surface }]}>
          ...
          >
            ...
            >
              <Text style={[s.dailyValue, { color: theme.text }]}>
                {avgDailyNew}
              </Text>
              <Text style={[s.dailyLabel, { color: theme.textMuted }]}>
                {ac?.avgDaily || 'Avg / day'}
              </Text>
            </View>
          ...
          <View style={{ marginTop: 12 }}>
            ...
            >
              <Text style={{ fontSize: 10, color: theme.textMuted }}>
                {ac?.day || 'Day'} {dayOfMonth}/{daysThisMonth}
              </Text>
              <Text style={{ fontSize: 10, color: theme.textMuted }}>
                {Math.round((dayOfMonth / daysThisMonth) * 100)}%
              </Text>
            </View>
            <View
              style={{
            ...
            >
              <View
                style={{
                  height: '100%',
                  borderRadius: 3,
                  width: `${(dayOfMonth / daysThisMonth) * 100}%`,
                  backgroundColor: theme.accent,
                }}
              />
            </View>
        ...
        ════════════════════════════════════════ */}
        <View
          style={[s.kpiGrid, { flexDirection: isRtl ? 'row-reverse' : 'row' }]}
        >
          <KpiCard
          ...
          />
          <KpiCard
            label={ac?.activeKpi || 'Active Users'}
            value={stats?.activeUsers || 0}
            trend={activeUserTrend - 50}
            color="#fff"
            bgColor="#22c55e"
        ...
        >
          <KpiCard
            label={ac?.plansKpi || 'Reading Plans'}
        ...
        {triviaOverview && (
          <TouchableOpacity
            style={[s.sectionCard, { backgroundColor: theme.surface }]}
            activeOpacity={0.8}
            onPress={() => navigation.navigate(route.adminTriviaPerformance as never)}
          >
            <View style={[s.settingsHeader, { flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
              ...
              <View style={{ flex: 1 }} />
              <Text style={{ fontSize: 12, color: theme.accent, fontWeight: '700' }}>View →</Text>
            </View>
            <View style={[s.dailyGrid, { flexDirection: isRtl ? 'row-reverse' : 'row', marginTop: 8 }]}>
              <View style={[s.dailyMetric, { alignItems: isRtl ? 'flex-end' : 'flex-start' }]}>
                <Text style={[s.dailyValue, { color: theme.text }]}>{triviaOverview.totalParticipants}</Text>
                <Text style={[s.dailyLabel, { color: theme.textMuted }]}>Participants</Text>
              </View>
              <View style={[s.dailyMetric, { alignItems: 'center' }]}>
                <Text style={[s.dailyValue, { color: theme.text }]}>{triviaOverview.totalAnswers}</Text>
                <Text style={[s.dailyLabel, { color: theme.textMuted }]}>Answers</Text>
              </View>
              <View style={[s.dailyMetric, { alignItems: isRtl ? 'flex-start' : 'flex-end' }]}>
                <Text style={[s.dailyValue, { color: theme.text }]}>{triviaOverview.averageScore}%</Text>
                <Text style={[s.dailyLabel, { color: theme.textMuted }]}>Avg Score</Text>
              </View>
            </View>
          </TouchableOpacity>
        )}
        ...
        <View style={[s.sectionCard, { backgroundColor: theme.surface }]}>
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              >
                {ac?.freeTranslationsDesc ||
                  'Limit Bible readers to free translations'}
              </Text>
            </View>
            <Switch
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              >
                {ac?.defaultTranslationDesc ||
                  'Translation to use by default in the Bible reader'}
              </Text>
            </View>
            <TouchableOpacity
